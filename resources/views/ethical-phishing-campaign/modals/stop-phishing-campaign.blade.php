<section class="text-sky-800">
    ...
    <p class="text-xl font-semibold text-center">
        @lang('ethical-phishing-campaign.modals.stop')
    </p>
    <div class="pt-4">
        <form id="stopForm" data-id=""
            action="{{ route('ethical-phishing-campaign.change-state', ['phishingCampaign' => 'id', 'state' => 'Finished']) }}"
            method="POST">
            @csrf
            @method('post')
            <p class="pb-4">@lang('ethical-phishing-campaign.modals.stopMessage')</p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        stopButton.forEach(button => {
            button.addEventListener('click', function() {
                ...
                const actionUrl =
                    "{{ route('ethical-phishing-campaign.change-state', ['phishingCampaign' => 'id', 'state' => 'Finished']) }}";
